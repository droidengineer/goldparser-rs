...
pub trait GPParser {
    ...
    fn input_token(&mut self) -> Token<T>;
...
pub struct Parser<T:Default+Clone> {
    ...
    pub input_tokens: Stack<Token<T>>,
...
impl<T:Default+Clone> Parser<T> {
    ...
    pub fn get_current_token(&self) -> Option<&Token<&T>> {
    ...
    pub fn produce_token(&mut self) -> Token<T> {
        trace!("produce_token");
...
impl GPParser for Parser {
    ...
    fn parse_token(&mut self, input_token: &mut Token<T>) -> GPParseResult {
        ...
        match parse_action.action {
            ...
            ActionType::Reduce => {
                ...
                if self.trim_reductions && rule.contains_one_nonterminal() {
            ...
            ActionType::Shift  => {
                ...
                input_token.lalr_state = self.curr_state as u16;
...
pub mod test {
    ...
    fn produce_token(parser: &mut Parser) -> crate::engine::token::Token<T> {
